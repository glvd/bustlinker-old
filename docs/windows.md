...
cd %GOPATH%/src/github.com/glvd/bustlinker/cmd/ipfs
```
...
```
go install -ldflags="-X "github.com/glvd/bustlinker".CurrentCommit=%SHA%"
%GOPATH%\bin\ipfs.exe version --all
```
...
If you encounter a bug with `go-ipfs` itself (not related to building) please use the [issue tracker](https://github.com/glvd/bustlinker/issues) to report it.
