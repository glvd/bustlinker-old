...
cd %GOPATH%/src/github.com/glvd/starship/cmd/ipfs
```
...
```
go install -ldflags="-X "github.com/glvd/starship".CurrentCommit=%SHA%"
%GOPATH%\bin\ipfs.exe version --all
```
...
If you encounter a bug with `go-ipfs` itself (not related to building) please use the [issue tracker](https://github.com/glvd/starship/issues) to report it.
