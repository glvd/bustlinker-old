...
**[`UnixfsAPI.Add()`](https://github.com/glvd/starship/blob/v0.4.18/core/coreapi/unixfs.go#L31)** - *Entrypoint into the `Unixfs` package*
...
- **[`adder.AddAllAndPin(files)`](https://github.com/glvd/starship/blob/v0.4.18/core/coreunix/add.go#L403)** - *Entrypoint to the `Add` logic*
  ...
  - **[`adder.addFile(file files.File)`](https://github.com/glvd/starship/blob/v0.4.18/core/coreunix/add.go#L450)** - *Create the _DAG_ and add to `MFS`*

      The `addFile(file)` method takes the data and converts it into a __DAG__ tree and adds the root of the tree into the `MFS`.

      https://github.com/glvd/starship/blob/v0.4.18/core/coreunix/add.go#L508-L521

      There are two main methods to focus on -

      1. **[`adder.add(io.Reader)`](https://github.com/glvd/starship/blob/v0.4.18/core/coreunix/add.go#L115)** - *Create and return the **root** __DAG__ node*
      ...
      2. **[`adder.addNode(ipld.Node, path)`](https://github.com/glvd/starship/blob/v0.4.18/core/coreunix/add.go#L366)** - *Add **root** __DAG__ node to the `MFS`*
  ...
  - **[`adder.Finalize()`](https://github.com/glvd/starship/blob/v0.4.18/core/coreunix/add.go#L200)** - *Fetch and return the __DAG__ **root** from the `MFS` and `UnixFS` directory*

      The `Finalize` method returns the `ipld.Node` from the `UnixFS` `Directory`.

  - **[`adder.PinRoot()`](https://github.com/glvd/starship/blob/v0.4.18/core/coreunix/add.go#L171)** - *Pin all files under the `MFS` **root***
